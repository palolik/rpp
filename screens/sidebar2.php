...
<div class="tamoto" id="myForm">
  <div style="display: flex;
    ...
    <button style=" background-color:white;border: none;padding:1px;cursor: pointer;"  onclick="closeForm()"><img style="height:20px" src="../clo.png"></button> 
        ...
        <textarea style="margin:20px;padding:10px;resize:none;color:black;font-size:20px;width:-webkit-fill-available;height:300px;background-color:white;" type="text" name="request" value=""><?php require_once("../config/database.php");$resultw = mysqli_query($mysqli, "SELECT * FROM employees where employeeid=180011266"); while($res = mysqli_fetch_assoc($resultw)) { echo  $res['email']  ;} ?><?php echo date("F jS, Y, h:i A"); ?>
